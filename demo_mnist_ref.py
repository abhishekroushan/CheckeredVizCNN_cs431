...
class CheckeredCNN(nn.Module):
        ...
        def __init__(self):
            ...
            self.init_conv = nn.Conv2d(1, 32, kernel_size=3, stride=1, padding=1)
            self.conv1 = nn.Conv2d(32, 32, kernel_size=3, stride=1, padding=1)
            self.conv2 = nn.Conv2d(32, 45, kernel_size=3, stride=2, padding=1)
            self.conv3 = nn.Conv2d(45, 45, kernel_size=3, stride=1, padding=1)
            self.conv4 = nn.Conv2d(45, 64, kernel_size=3, stride=1, padding=1)
...
def run(data_path, n_epochs=100):
    ...
    for epoch in range(1, n_epochs + 1):
        ...
        torch.save(model.state_dict(), MODEL_STORE_PATH +'ref_conv_net_model.ckpt')
        print("Model saved after epoch at ", MODEL_STORE_PATH )
